...
const CharacterTab: React.FC<{ engine: GameEngine; power: number; canLevelUp: boolean; onLevelUp: () => void; inventory: Record<string, number>; refresh: () => void; onEquipment: (slot: EquipSlotKey, key: string) => void }> = ({ engine, power, canLevelUp, onLevelUp, inventory, refresh, onEquipment }) => {
  ...
  const scores = ['partitura_bronze', 'partitura_prata', 'partitura_ouro'];
  return <div className="flex h-full min-h-0">
    ...
    <section className="ac-scroll-region ac-character-detail min-w-0 flex-1 overflow-y-auto overflow-x-hidden p-[clamp(12px,1.4vw,24px)]">
      ...
      <Panel className="mt-3 flex items-center gap-4 p-3"><div className="min-w-0 flex-1"><h3 className="text-sm font-black text-white">Partituras para evolução</h3><p className="mt-0.5 text-[10px] text-[#7f93b5]">Subir nível consome apenas partituras. Materiais extras pertencem somente à Ascensão.</p><div className="mt-2 flex gap-2">{scores.map((itemKey) => { const meta = ITEM_META[itemKey]; return <div key={itemKey} className="flex min-w-[86px] items-center gap-2 rounded-lg border border-[#29415e] bg-[#071326] px-2 py-1.5"><img src={meta.img} alt="" className="h-8 w-8 object-contain"/><div><p className="text-[9px] text-[#91a4c4]">{meta.name.replace('Partitura de ', '')}</p><b className="text-xs text-white">{inventory[itemKey] || 0}</b></div></div>; })}</div></div><button type="button" disabled={!canLevelUp} onClick={() => { onLevelUp(); refresh(); }} className={`min-w-[190px] rounded-xl px-5 py-4 text-base font-black ${canLevelUp ? 'bg-gradient-to-r from-emerald-600 to-teal-500 text-white shadow-[0_0_20px_rgba(16,185,129,.25)]' : 'bg-[#183047] text-[#64748b]'}`}><ArrowUp className="mr-2 inline h-5 w-5" />Subir de Nível</button></Panel>
    </section>
  </div>;
};
